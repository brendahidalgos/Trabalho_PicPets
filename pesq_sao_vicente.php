...
echo "<h2>São vicente"."</h2><br>";


  $rs = $conexao->query("select count(cidade_fotografo) as 'São_vicente' from cidade_fotografo
    where cidade_fotografo = 'São vicente'");

  while($row = $rs->fetch(PDO::FETCH_OBJ))
      {

        echo "<div class='pergunta'> Contem ".$row->São_vicente." Fotógrafos nessa cidade.</div><br><br>";
  ...
  $rs = $conexao->query("select nome_fotografo from cadastro_fotografo, endereco_fotografo, bairro_fotografo, cidade_fotografo
    where endereco_fotografo.idcadastro_fotografo = cadastro_fotografo.idcadastro_fotografo
      ...
      and cidade_fotografo = 'São vicente'");

  while($row = $rs->fetch(PDO::FETCH_OBJ))
      {

        echo "<div class='resposta'>".$row->nome_fotografo."</div><br><br>";
